      ...
      <div class="table-card card">
        <div class="table-header">
          <div class="left">
            <div class="title-row">
              ...
              <div class="titles">
                <div class="sub-title">Admin : Permission</div>
        ...
        <div class="table-body p-16">
          <section class="permission-page user-profile">
            ...
            <div class="per-table" v-for="(item, index) in storeVar.permissiondata" :key="index">
              ...
              <div class="right-permissions">
                <div class="input-row">
                  <div class="toggle-input">
                    <p>All</p>
                    <label class="toggle-control">
                      <input type="checkbox" v-model="item.allStatus" @change="checkedAll(item.id, item.allStatus)">
                      <span class="control"></span>
                    </label>
                  </div>
                </div>
                <div class="input-row" v-for="(perms, permsIndex) in item.userPermission" :key="permsIndex">
                  <div class="toggle-input">
                    <p>{{ perms.permission?.name }}</p>
                    <label class="toggle-control">
                      <input type="checkbox" v-model="perms.status" @change="checkedAllOrNot(item.id)">
                      <span class="control"></span>
                    </label>
                  </div>
                </div>
          ...
          <div class="btns justify-end mt-10">
            <div class="btn white-btn" @click="updatePermission">Save Changes</div>
          </div>
        </div>
      </div>
    </div>
...
function updatePermission() {
  store.dispatch("Staff/updatePermission", { menu: storeVar.value.permissiondata });
}
function checkedAll(id, status) {
  const objIndex = storeVar.value.permissiondata.findIndex((obj) => obj.id === id);
  if (objIndex >= 0) {
