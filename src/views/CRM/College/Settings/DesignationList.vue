        ...
        <div class="right">
          ...
          <div class="icons">
            <div
              ...
              @click.prevent="generateFinalCard()"
            >
              <icon-card></icon-card>
              <div class="tooltip">Final Card</div>
            </div>
          </div>
          <div class="icons">
            <div
      ...
      <div class="table-body mt-16">
        <div class="table-outer">
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, index) in storeVar.designation" :key="index">
                <!-- <td>{{ index + 1 }}</td> -->
                <td>
                  <input
                    type="checkbox"
        ...
        <div class="table-no-data" v-if="storeVar.designation?.length <= 0">
...
const assignProduct = () => {
  if (formVar.ids.length > 0 && formVar.idCardStockId) {
    ...
    store.dispatch("OrganizationSettings/assignDesignationProduct", payload);
  } else {
    store.dispatch("Alert/error", {
      msg: "Either no designation or select product!",
    });
  }
};
...
function addDesignation() {
  ...
  if (!formVar.editMode) {
    store.dispatch("OrganizationSettings/addDesignation", {
      name: formVar.name,
      priority: formVar.priority,
      settingId: formVar.organizationSettingId,
    });
  } else {
    store.dispatch("OrganizationSettings/updateDesignation", {
...
function submitDelete() {
  store.dispatch("OrganizationSettings/deleteDesignation", {
    id: formVar.designationId,
    status: "DELETED",
  });
}
