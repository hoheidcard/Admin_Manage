      ...
      <div class="">
        <div class="table-header">
          <div class="left">
            <div class="title-row">
              ...
              <div class="titles">
                <div class="sub-title">Partner : Permission</div>
        ...
        <div class="table-body mt-16">
          <section class="permission-page user-profile">
            ...
            <div
              class="per-table"
              v-for="(item, index) in storeVar.partnerPermissionData"
              :key="index"
            >
...
function updatePermission() {
  store.dispatch("Staff/updatePermission", {
    menu: storeVar.value.partnerPermissionData,
  });
}
function checkedAll(id, status) {
  const objIndex = storeVar.value.partnerPermissionData.findIndex(
    (obj) => obj.id === id
  );
  if (objIndex >= 0) {
    let userPerms =
      storeVar.value.partnerPermissionData[objIndex].userPermission;
...
function checkedAllOrNot(id) {
  ...
  if (objIndex >= 0) {
    ...
    storeVar.value.partnerPermissionData[objIndex].allStatus = allTrue;
  }
}
function selectAll() {
  if (formVar.all) {
    storeVar.value.partnerPermissionData.forEach((element) => {
